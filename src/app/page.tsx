...
export default async function DashboardPage() {
  ...
  const [summaryRes, incomeRes, recentRes] = await Promise.all([
    supabase
  ...
  const summary = (summaryRes.data ?? []) as MonthlySummaryRow[];
  const incomes = (incomeRes.data ?? []) as Income[];
  const recent = (recentRes.data ?? []) as unknown as (Expense & {
  ...
  return (
    <PageShell title={`Hai, ${displayName} 👋`} subtitle={monthLabel()}>
      {/* Hero summary */}
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <Link href="/settings" className="text-xs text-brand-600">Atur</Link>
        </div>
        <div className="card divide-y divide-slate-100 p-0">
          {summary.length === 0 && (
            <p className="p-4 text-sm text-slate-500">Belum ada kategori. Buat di Pengaturan.</p>
          )}
          {summary.map((row) => {
        ...
        <div className="card divide-y divide-slate-100 p-0">
          {recent.length === 0 && (
            <p className="p-4 text-sm text-slate-500 text-center">
              Belum ada pengeluaran. Tap tombol <span className="font-medium">Catat</span> di bawah!
            </p>
          )}
          {recent.map((e) => (
            <div key={e.id} className="flex items-center justify-between p-4">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500 ml-4 mt-0.5">
                  {e.categories?.name} · {new Date(e.spent_at).toLocaleDateString("id-ID", {
                    day: "numeric",
                    month: "short",
                  })}
                </p>
